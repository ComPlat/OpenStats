...
$.extend(expressionBinding, {
  ...
  initialize: function(el) {
    ...
    // Tell the server this binding has (re)mounted so it re-sends its payload
    // (operator set / docs / specs). Covers modal reopen, tab switch, etc.
    if (typeof Shiny !== "undefined" && Shiny.setInputValue) {
      Shiny.setInputValue(el.id + "-ready", Date.now(), { priority: "event" });
    }
  },
  getId: function(el) {
    return el.id;
