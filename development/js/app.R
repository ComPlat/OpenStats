...
data_wrangling_specs <- function() {
  list(
    ...
    quantile = list(min_args = 1, max_args = 2),
    ...
    scale = list(min_args = 1, max_args = 1),
    length = list(min_args = 1, max_args = 1),
    vector = list(min_args = 1, max_args = 2),
    matrix = list(min_args = 1, max_args = 3),
    DataFrame = list(min_args = 1),
    C = list(min_args = 1),
    Seq = list(min_args = 2, max_args = 3),
    get_rows = list(min_args = 2, max_args = 2),
    get_cols = list(min_args = 2, max_args = 2),
    get_elem = list(min_args = 3, max_args = 3)
  )
}
...
expr_builder_server <- function(id, state, refresh = shiny::reactive(NULL), check_variables, operator_set, docs, specs) {
  shiny::moduleServer(id, function(input, output, session) {
    shiny::observeEvent(list(state$df, refresh()), {
      types <- lapply(state$df, function(col) {
        c <- class(col)
        if (any(c != "numeric")) return(paste(c, collapse = " "))
        qs <- quantile(col)
        res <- paste0(
          "numeric; ", "\n",
          "min: ", min(col), " ", "max: ", max(col), "\n",
          "median: ", median(col), " ", "mean: ", mean(col), "\n",
          "sd: ", sd(col), "\n",
          "1.Quartile: ", qs[[2L]], " ", "3.Quartile: ", qs[[4L]]
        )
      }) |> unname()
      session$sendInputMessage("expr", list(
        variables = names(state$df),
...
ui <- fluidPage(
  ...
  tags$head(
    includeCSS("~/Documents/OpenStats/development/js/styles.css"),
    includeScript("~/Documents/OpenStats/development/js/parser.js"),
    includeScript("~/Documents/OpenStats/development/js/script.js")
  ),
  uiOutput("test"),

  verbatimTextOutput("Bla"),
  shiny::actionButton("Open", "Open")
)
...
server <- function(input, output, session) {
  ...
  operations <- list(
    ...
    "Statistical & Utils Functions" = list(
      "Mean", "SD", "Median", "Sum",
      "C", "Seq", "DataFrame"
    ),
    Getter = list(
      "get_elem", "get_rows", "get_cols"
    )
  )
  state <- reactiveValues(op = operations, df = NULL, expr = NULL)

  refresh <- shiny::reactiveVal(0)
  expr_builder_server(
    "TEST", state, refresh = refresh, check_variables = TRUE,
  ...
  output$Bla <- shiny::renderPrint({
    paste0(input[["TEST-expr"]]$tokens, collapse = " ")
  })

  shiny::observeEvent(input$Open, {
    req(state$df)
    req(is.data.frame(state$df))
    shiny::showModal(shiny::modalDialog(
      expr_builder_ui("TEST", state$op, names(state$df)),
      easyClose = TRUE,
      size = "l",
