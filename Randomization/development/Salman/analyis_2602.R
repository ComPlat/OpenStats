...
groups <- read.csv("./Randomization/development/Salman/2602.csv", sep = ";", header = TRUE)
treatment_groups <- c("mCherry", "Gi", "Gq")
n_blocks <- data.frame(
  treatment = c(rep(treatment_groups[1:2], each = 11L), rep(treatment_groups[3], 10))
)
...
write.csv(res, file = "./Randomization/development/Salman/2602_result.csv", quote = FALSE)
